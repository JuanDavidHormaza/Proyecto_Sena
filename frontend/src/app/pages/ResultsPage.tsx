/**
 * ResultsPage.tsx  — VERSIÓN CORREGIDA
 * Cambios:
 *  1. Escala visual: Básico (A1-A2) | Intermedio (B1) | Avanzado (B2)  — elimina C1/C2
 *  2. Nivel mostrado: usa levelReached del QuizPage (nivel real asignado) en lugar
 *     de getLevelFromScore aplicado al puntaje total.
 *  3. getLevelFromScore importado de levelUtils (fuente única de verdad).
 */
...
// ✅ Importar desde levelUtils (fuente única de verdad)
import { getLevelInfoByCode, LEVEL_BANDS } from "../utils/levelUtils";
...
type StoppedReason = "passed_all" | "level_assigned" | "below_minimum" | undefined;
...
export function ResultsPage() {
  ...
  const finalScore =
    resultsState?.score ?? parseInt(localStorage.getItem("quizScore") || "0");
  const correctAnswers =
    resultsState?.correctAnswers ??
    parseInt(localStorage.getItem("correctAnswers") || "0");
  const totalQuestions =
    resultsState?.totalQuestions ??
    parseInt(localStorage.getItem("totalQuestions") || "0");
  const answers = resultsState?.answers ?? [];
  const duration =
    resultsState?.duration ?? localStorage.getItem("quizDuration") ?? "00:00";
  const stoppedReason: StoppedReason = resultsState?.stoppedReason;

  // ✅ CORRECCIÓN: usamos el nivel asignado por el QuizPage, no el derivado del score
  const assignedLevel = resultsState?.levelReached ?? localStorage.getItem("assignedLevel") ?? "";
  const levelInfo = getLevelInfoByCode(assignedLevel);
  ...
  const getGradientColors = () => {
    if (finalScore >= 90) return "from-sena-green to-sena-green-dark";
    if (finalScore >= 60) return "from-warning to-amber-600";
    return "from-destructive to-red-700";
  };

  // ── Banner de motivo de parada ─────────────────────────────────────────────
  const renderStoppedBanner = () => {
    if (!stoppedReason || stoppedReason === "passed_all") return null;

    if (stoppedReason === "below_minimum") {
      return (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.45 }}
          className="flex items-start gap-3 rounded-2xl border border-orange-300 bg-orange-50 px-5 py-4 mb-6"
        >
          <AlertTriangle className="w-5 h-5 text-orange-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-semibold text-orange-700">Puntaje por debajo del mínimo</p>
            <p className="text-sm text-orange-600">
              No alcanzaste el 60 % en este nivel. Tu nivel asignado corresponde al nivel
              anterior completado.
            </p>
          </div>
        </motion.div>
    ...
    if (stoppedReason === "level_assigned") {
      return (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.45 }}
          className="flex items-start gap-3 rounded-2xl border border-sena-blue/30 bg-sena-blue/5 px-5 py-4 mb-6"
        >
          <Sparkles className="w-5 h-5 text-sena-blue flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-semibold text-sena-blue">Nivel asignado</p>
            <p className="text-sm text-sena-blue/80">
              Obtuviste entre 60 % y 89 % en este nivel — ¡este es tu nivel actual de inglés!
            </p>
          </div>
        </motion.div>
      );
  ...
  const categoryPerformance = Object.values(
    answers.reduce<
      Record<string, { name: string; total: number; correct: number; score: number }>
    >((categories, answer) => {
  ...
  const writingAnswers = answers.filter((a) => a.writingAnswer).length;
  const speakingAnswers = answers.filter((a) => a.audioUrl).length;
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <section
        className={`relative overflow-hidden bg-gradient-to-br ${getGradientColors()} py-16 lg:py-24`}
      >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                {/* ✅ CORRECCIÓN: muestra el nivel real asignado */}
      ...
      <section className="container mx-auto max-w-4xl px-4 -mt-8 pb-12 relative z-10">
        ...
        >
          {[
            ...
            {
              label: "Incorrectas",
              value: `${Math.max(totalQuestions - correctAnswers, 0)}/${totalQuestions}`,
              icon: XCircle,
              color: "destructive",
            },
            ...
            >
              <div
                className={`w-10 h-10 bg-${stat.color}/10 rounded-xl flex items-center justify-center mb-3`}
              >
        ...
        >
          ...
          {categoryPerformance.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">
              No hay respuestas registradas para calcular categorias.
            </p>
          )}
        ...
        {/* ✅ CORRECCIÓN: Escala de Niveles — máximo B2, sin C1/C2 */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            {LEVEL_BANDS.map((item, index) => {
              const isActive = item.match(finalScore);
              return (
                <div
                  key={index}
                  ...
                  style={{
                    ...
                    ringColor: isActive ? item.color : undefined,
                  }}
                >
                  <span
